...
ruta <- "E:/SEGMENTACION/VARIABLES/E/ndvi/NDVI_Max_1984.tif"
ruta_temp <- "E:/R/RTemp"
...
if (file.exists(ruta)) {
  ...
  if (is.na(crs(raster))) {
    message("  CRS de raster_presencia indefinido. Asignando EPSG:32630...")
    crs(raster) <- "EPSG:32630" # Esto se comprobó empleando QGIS
  }
...
# Guardamos cada una de la capa
writeRaster(raster_rep, 
            "E:/SEGMENTACION/VARIABLES/E/ndvi_rep/ndvi_rep.tif", 
            overwrite = TRUE, 
            datatype = "FLT8S",
            gdal = c(
              "COMPRESS = DEFLATE",
